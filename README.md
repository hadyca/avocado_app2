...
- [] 사진 내리면 닫기
- [] 홈 화면 설명은 코딩 Text가 아닌 이미지로 넣기 (각 언어 별로 줄이 바뀌어서 이쁘지가 않음)
...
- [] 즐겨찾기 인스타 참조 (북마크)
- [] 백엔드 error 메시지 그대로 출력 대신에, error를 코드로 받고 프론트에서 text로 출력 (번역 때문에)
- [] 없는 게시글, 코멘트 에 뭔가 할 때 error 반환 -> alert로 삭제된 거라고 띄우기
- [] 안드로이드 alert 글씨가 길면 씹힘
  각 화면별로 디테일하게 다시 한번더 봐야함 누락된 부분 많음
- [] 스플래시화면(아이콘 화면이랑 동일하게), 비나 알바 로고화면 일부 수정, 앱 로고는 볼나온 다람쥐 일러스트(몽키비슷하게) (위아래 줄 없애기 : Text만)
- [] 계정 이메일 -> 모바일 번호로 변경
- [] 개인 프로필 화면 디자인 너무 구림 반드시 고쳐야함
- [] 모든 텍스트 화면 넘어가는 문제 해결하기
- [] 기업회원가입 : 이미 가입자: 최종화면 안보이게 다시 돌려놓기
- [x] 초기 i18n 에러 잡기
- [x] editProfile 부분 번역
