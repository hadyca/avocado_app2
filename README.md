...
- [] 알림 화면 만들기 (알람 종 모양 누르면, 페이지 이동) - Me에만 세팅(profile화면은 그대로두기)
- [] 키보드 높이 조절( 안드로이드, 자동 완성 기능 있을 때 가려짐)
- [] 개인 설정 화면 만들기
- [] 다국어 세팅 + data 전달 시 유저가 직접 매뉴얼하게 입력하는 title, content 제외, 선택 할 수 있는 data들은 전부 영문으로 저장 되게끔 세팅 (나중 데이터 분석 및 필터링을 위해)
- [] 전체 폰트 크기 일괄 조정
- [] Header 스크린명 정하기
- [] Home 화면완성 (범구 진행 중)
