...
- [] Home화면에서 PushToken값 조회 후, 스토리지에 저장 --> 로그아웃 할 때 해당 값 끌고와서 DB 삭제해주기

- [] 백엔드 pushToken 유니크 값 하지말자 --> 로그아웃 말고, 그냥 앱 삭제하고 다시 설치하면 해당 pushToken값이 그대로 있음 삭제가 안되고(유니크값 중복 에러 뜸),,, 로긴 후, 로그아웃 안하고 바로 앱 삭제한 다음에 다른 계정으로 로그인 하게 된다면???? 기존 {id:1, pushToken: "abc"}가 있는 상태에서 {id:2, pushToken: "abc"}로 로그인하게 되면, id:1에게 보낼 알림이 id:2 에게 가게 된다... 문제된다.. 앱 삭제 할때 알 수있는 방법이 있나?

아니다 백엔드 pushToken 유니크 해야 된다 --> pushToken 1개가 여러 username이 쓸 수 없게, 그렇다면? --> confirm secret /로그인에서 성공 하게 될 때, 기존
pushToken값 조회 후 있다면 삭제 하고 나서, 새롭게 생성하는 로직으로 해야된다!

- [] 알람 요청 거절 후, 다시 승인 하고 나서 pushtoken 받게끔 세팅
- [] 알람 설정 화면 만들기 (전체 알람, 코멘트 알람, 좋아요 알람, 팔로잉 알람, 공지 사항 알람 총 4가지 switch로 만들기 - 백엔드 user별로 각 유형 별 알람 boolean으로 설정 true면 백엔드에서 db에 저장하고, false면 저장 안하는 식으로 + 저장이 안되고 알람도 안가게끔.. 2개 해야함 저장과 알람 모두 안하게)
- [] 백엔드 mutation 부분은 전부 protected로 감싸기
...
- [] 백,프론트 기업 탈퇴 만들기 (user는 탈퇴 없음)
- [] 모든 텍스트 화면 넘어가는 문제 해결하기
- [] 아이폰 다크 모드 일때 placeholder 글씨가 안보이는 문제 - 직접 컬러 지정해주기
- [] Notification 30일 이상 지난 것은 자동삭제 되게끔 세팅(하루에 한번씩 setInterval로 백엔드에서 처리)
- [] 신고하기 : 기타 부분에 직접 Text 쓸수 있게 추가
- [] 인증 문자 보낼 때 Text 수정 (복사 붙여넣기 하라는말 빼기 / 실제 앱 서비스명 사용)
...
- [] 사진 클릭 시 사진 확대 및 여러 사진 스와이프 기능 (프로필 사진 포함)
...
- [] Welcome : 계정 메일 찾기, 비 찾기 기능 추가 // 페이스북 로그인 기능 추가
