...
- [] 알람 설정 화면 만들기 (전체 알람, 코멘트 알람, 좋아요 알람, 공지 사항 알람 총 4가지 switch로 만들기 - 백엔드 user별로 각 유형 별 알람 boolean으로 설정 true면 백엔드에서 db에 저장하고, false면 저장 안하는 식으로 + 저장이 안되고 알람도 안가게끔.. 2개 해야함 저장과 알람 모두 안하게)
