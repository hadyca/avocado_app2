...
- [] 기기에서 언어 끌고 오는걸로 기본 언어 세팅, 그리고 이걸 각 유저 db 값에 던져주고 유저별 저장된 언어로 백엔드에서 알람 보내주기, 그리고 언어 설정에서 언어 바꾸면 기기 언어도 바꾸는걸로 가능할까?
- [] expo token 자꾸 에러뜬다면,,? 아마도 remove 해줘야할듯 useEffect 에서 (공식 문서 참조)
